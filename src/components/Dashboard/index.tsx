...
import { ButtonWrapper, Content, DetailsWrapper, ErrorMessage, ExpandSpan, FoundUrl, InputWrapper, Keyword, NotFound, ResultItem, Results, SeachKeyStatus, SearchLabel, Tag, UrlList } from './styles';
...
const Dashboard: React.FC = () => {
    ...
    return <Content> 
          <InputWrapper>
            <Input name="Search" value={search} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearch(e.target.value)} placeholder="Search for keywords" />
            <Button onClick={() => verifyInput(search)}>
                <SearchLabel>Search</SearchLabel>
            </Button>
          </InputWrapper>
          {errorMessage ? (<ErrorMessage>{errorMessage}</ErrorMessage>) : null}
          {result.length > 0 && <Results className="results">
              {result.map((item, index) => {
                  return (
                      <React.Fragment key={index}>
                        <ResultItem className="result-item" key={index}>
                          <SeachKeyStatus>
                          <Tag status={item.status}>{item.status.toUpperCase()}</Tag>
                          <Keyword>{item.keyword}</Keyword>
                          </SeachKeyStatus>
                          <ExpandSpan onClick={() => changeExpanded(index)}>{item.expanded ? "hide urls" : "show urls"}</ExpandSpan>
                          </ResultItem>
                          {item.expanded ? (
                              <DetailsWrapper>
                                    <UrlList>
                                  ...
                                  item.urls.map((url, index) => {
                                      return (
                                          <FoundUrl className="found-url" key={index}>{url}</FoundUrl>
                                      )
